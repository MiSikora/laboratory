...
internal fun <T> Iterable<T>.containsAllInOrder(
  ...
  val allCombinations =
    asSequence().mapIndexed(::Pair).flatMap { left ->
      other.mapIndexed { index, right -> left to (index to right) }
    }
  val uniqueFinds =
    allCombinations
...
internal fun Iterable<String>.containsAllInOrder(other: Iterable<String>) =
  containsAllInOrder(other) { text, query -> text.contains(query, ignoreCase = true) }
