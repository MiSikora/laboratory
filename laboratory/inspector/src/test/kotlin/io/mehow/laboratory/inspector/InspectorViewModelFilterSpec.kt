...
class InspectorViewModelFilterSpec :
  FunSpec({
    setMainDispatcher()
    ...
    test("finds feature flags by split name parts") {
      ...
      InspectorViewModel(searchFlow).observeFeatureClasses().test {
        ...
        awaitItem() shouldContainExactly
          listOf(Numbered1NameFeature::class, RegularNameFeature::class)
    ...
    // It fails randomly with a timeout on a second event. Re-using generator seed does not help
    // locally.
...
private enum class SourcedFeature : Feature<SourcedFeature> {
  ...
  @Suppress("UNCHECKED_CAST") override val source = Source::class.java as Class<Feature<*>>
...
private fun InspectorViewModel(searchFlow: Flow<SearchQuery>) =
  InspectorViewModel(
    Laboratory.inMemory(),
    searchFlow,
...
private suspend fun TurbineTestContext<List<KClass<out Feature<*>>>>.expectAllFeatureFlags() {
  awaitItem() shouldContainExactly
    listOf(Numbered1NameFeature::class, RegularNameFeature::class, SourcedFeature::class)
}
