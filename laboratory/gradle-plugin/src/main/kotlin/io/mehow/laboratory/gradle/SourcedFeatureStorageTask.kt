...
internal abstract class SourcedFeatureStorageTask @Inject constructor(objects: ObjectFactory) :
  OutputTask() {
  @Input
  @Optional
  val storage: Property<SourcedFeatureStorageInput> =
    objects.property(SourcedFeatureStorageInput::class.java)
  ...
  fun generateSourcedFeatureStorage() {
    ...
    storage.orNull
      ?.toModel(
        features.get().flatMap(FeatureFlagInput::toModelsWithChildren).sourceNames().distinct()
      )
      ?.prepare()
      ?.writeTo(outputDirectory.get().asFile)
  }

  private fun List<FeatureFlagModel>.sourceNames(): List<String> =
