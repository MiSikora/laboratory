...
public class ChildFeatureFlagsInput
internal constructor(
  ...
  public fun feature(name: String, action: Action<FeatureFlagInput.MultiOption>) {
    featureInputs +=
      FeatureFlagInput.MultiOption(name, packageNameProvider, supervisor).apply(action::execute)
  }
  ...
  private fun binaryFeature(
    ...
    featureInputs +=
      FeatureFlagInput.BinaryOption(name, isEnabled, packageNameProvider, supervisor)
        .apply(action::execute)
  }

  internal fun toModels(): List<FeatureFlagModel> =
    featureInputs.flatMap(FeatureFlagInput::toModelsWithChildren)
