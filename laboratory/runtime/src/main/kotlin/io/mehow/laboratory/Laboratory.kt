...
public class Laboratory internal constructor(builder: Builder) {
  ...
  public fun <T : Feature<out T>> observe(feature: Class<out T>): Flow<T> =
    observeRaw(feature as Class<Feature<*>>) as Flow<T>
  ...
  public suspend fun <T : Feature<out T>> experiment(feature: Class<out T>): T =
    experimentRaw(feature as Class<Feature<*>>) as T
  ...
  public suspend fun <T : Feature<out T>> experimentIs(option: T): Boolean =
    experimentRaw(option::class.java as Class<Feature<*>>) == option
   ...
   * option for the same feature flag, the last one should be applied.
   *
   * @return `true` if the option was set successfully, `false` otherwise.
   */
  ...
  private fun <T : Feature<out T>> getDefaultOption(feature: Class<out T>) =
