...
internal class DataStoreFeatureStorage(private val dataStore: DataStore<FeatureFlags>) :
  FeatureStorage {
  override fun observeFeatureName(feature: Class<out Feature<*>>) =
    dataStore.data.map { it.options[feature.name] }
  ...
  private suspend fun setOptions(options: Map<String, String>) =
    try {
